...
fn build_native() {
    ...
    let config_libs = String::from_utf8(config_output.stdout).expect("unable to parse wx-config libs output");
    let linker_flags: Vec<String> = config_libs.split(" ").into_iter().filter_map(|s| {
        if !s.trim().is_empty() {
            Some(s.trim().to_owned())
        }else{
            None
    ...
    for (i, flag) in linker_flags.iter().enumerate() {
        if flag.starts_with("-L") {
            ...
            let captures = static_lib_extract.captures(flag).expect("unable to capture flag regex");
